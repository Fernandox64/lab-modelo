\documentclass[12pt,a4paper]{article}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[brazil]{babel}
\usepackage{geometry}
\usepackage{longtable}
\usepackage{array}
\usepackage{hyperref}
\geometry{margin=2.5cm}

\title{Documentacao Tecnica do Portal DECOM/UFOP}
\author{Projeto DECOM - Site Institucional}
\date{\today}

\begin{document}
\maketitle
\tableofcontents
\newpage

\section{Objetivo do documento}
Este documento descreve, de forma consolidada, o que foi implementado no portal institucional do DECOM/UFOP ate o momento. O foco e registrar:
\begin{itemize}
    \item arquitetura adotada;
    \item modelos de dados implementados;
    \item recursos visuais (CSS e componentes);
    \item menus publicos e administrativos;
    \item motivos tecnicos para uma arquitetura limpa, leve e de baixo consumo de recursos.
\end{itemize}

\section{Visao geral da solucao}
O sistema foi estruturado como um portal PHP com Docker, com persistencia em MySQL e painel administrativo para secretaria e equipe interna.

\subsection{Princpios arquiteturais}
\begin{itemize}
    \item \textbf{Arquitetura clean e leve}: separacao clara entre camada de apresentacao (paginas PHP), regras de negocio utilitarias (funcoes em \texttt{includes/config.php}) e dados (MySQL).
    \item \textbf{Simplicidade operacional}: execucao em containers com poucos servicos e configuracao direta.
    \item \textbf{Baixo custo computacional}: sem framework pesado no runtime principal; paginas renderizadas no servidor com consultas simples e indexadas.
    \item \textbf{Manutenibilidade}: painel admin para reduzir alteracoes manuais em codigo (conteudo, menu, pos-graduacao, egressos, noticias, editais etc.).
\end{itemize}

\subsection{Motivacao para a escolha}
A arquitetura foi escolhida para ambientes de servidor com recursos simples, onde e importante:
\begin{itemize}
    \item consumir pouca memoria e CPU;
    \item reduzir complexidade de deploy;
    \item facilitar suporte tecnico local;
    \item manter tempo de resposta adequado com stack enxuta.
\end{itemize}

\section{Stack e software livre utilizado}
\subsection{Infraestrutura}
\begin{itemize}
    \item \textbf{Docker / Docker Compose} para orquestracao.
    \item \textbf{PHP 8.2 + Apache} como aplicacao web.
    \item \textbf{MySQL 8 Community} como banco de dados.
    \item \textbf{phpMyAdmin} para administracao do banco.
\end{itemize}

\subsection{Frontend e UI}
\begin{itemize}
    \item \textbf{Bootstrap 5} (CDN) para layout responsivo.
    \item \textbf{AdminLTE 4} (CDN) para painel administrativo.
    \item \textbf{TinyMCE} (modo gratuito/comunitario) para edicao rica de conteudo no admin.
\end{itemize}

\subsection{Observacao de licenciamento}
Foi priorizado o uso de tecnologias abertas, amplamente adotadas e com opcoes gratuitas de uso em ambiente institucional, favorecendo sustentabilidade tecnica e baixo custo.

\section{Arquitetura de pastas (principal)}
\begin{itemize}
    \item \texttt{app/}: codigo principal do portal.
    \item \texttt{app/includes/}: configuracao global e layout base (header/footer).
    \item \texttt{app/admin/}: painel administrativo.
    \item \texttt{app/noticias/}, \texttt{app/ensino/}, \texttt{app/decom/}, \texttt{app/pesquisa/}, \texttt{app/pessoal/}: modulos publicos.
    \item \texttt{app/scripts/}: scripts de importacao/populacao.
    \item \texttt{mysql-init/}: estrutura inicial do banco e seeds.
\end{itemize}

\section{Modelos implementados (banco de dados)}
As tabelas principais implementadas incluem:

\begin{longtable}{|p{5cm}|p{9cm}|}
\hline
\textbf{Tabela} & \textbf{Finalidade} \\
\hline
\texttt{news_items} & Noticias institucionais \\
\hline
\texttt{edital_items} & Editais gerais do departamento \\
\hline
\texttt{defesa_items} & Defesas (TCC, mestrado, doutorado etc.) \\
\hline
\texttt{job_items} & Estagios e empregos \\
\hline
\texttt{people_items} & Docentes e funcionarios (com dados de perfil e foto) \\
\hline
\texttt{research_labs} & Laboratorios de pesquisa \\
\hline
\texttt{research_projects} & Projetos de pesquisa/extensao \\
\hline
\texttt{site_settings} & Configuracoes editaveis do menu principal \\
\hline
\texttt{ppgcc_page_content} & Conteudo institucional da pagina de pos-graduacao \\
\hline
\texttt{ppgcc_graduates} & Formandos/egressos da pos por ano \\
\hline
\texttt{ppgcc_notices} & Editais e informes especificos da pos-graduacao \\
\hline
\texttt{ppgcc_selection_items} & Itens importados da pagina oficial de processo seletivo do PPGCC (agrupados por edital/selecao) \\
\hline
\end{longtable}

\section{CSS e padrao visual utilizado}
\subsection{CSS base do site publico}
O portal utiliza Bootstrap 5 com customizacao leve e centralizada no arquivo:
\begin{center}
\texttt{app/assets/css/theme.css}
\end{center}
Esse arquivo foi criado para facilitar manutencao de cores em um unico local.

Paleta oficial configurada (variaveis CSS):
\begin{itemize}
    \item \texttt{\#FFDE42} / \texttt{rgb(255, 222, 66)}
    \item \texttt{\#53CBF3} / \texttt{rgb(83, 203, 243)}
    \item \texttt{\#5478FF} / \texttt{rgb(84, 120, 255)}
    \item \texttt{\#111FA2} / \texttt{rgb(17, 31, 162)}
\end{itemize}

O \texttt{header.php} apenas referencia esse CSS global, incluindo:
\begin{itemize}
    \item topbar institucional;
    \item navbar principal (Bootstrap) com paleta do DECOM;
    \item cards com sombra suave (\texttt{news-card});
    \item capas de imagem padronizadas (\texttt{news-card-cover});
    \item cores consistentes para identidade institucional.
\end{itemize}

\subsection{Como alterar cores rapidamente}
Para mudar a identidade visual no futuro, editar somente o bloco \texttt{:root} em:
\begin{center}
\texttt{app/assets/css/theme.css}
\end{center}
As variaveis (\texttt{--decom-yellow}, \texttt{--decom-cyan}, \texttt{--decom-blue}, \texttt{--decom-navy}) propagam automaticamente para menu, botoes, titulos e cards.

\subsection{CSS do painel admin}
No admin foi adotado AdminLTE 4 com Bootstrap 5 para:
\begin{itemize}
    \item sidebar fixa;
    \item navegação por modulos;
    \item formularios administrativos padronizados;
    \item tabela de gerenciamento (CRUD).
\end{itemize}

\section{Menus implementados}
\subsection{Menu publico (site)}
Menu principal em \texttt{app/includes/header.php}, com secoes:
\begin{itemize}
    \item Inicio
    \item DECOM
    \item Noticias e Eventos
    \item Pessoal
    \item Ensino (incluindo Pos-graduacao em Computacao)
    \item Processo Seletivo (PPGCC) no menu de Ensino
    \item Graduacao (editavel)
    \item Pos-graduacao (editavel)
    \item Pesquisa
    \item Extensao
    \item Contato
\end{itemize}

\subsection{Menu administrativo}
Painel administrativo com os modulos:
\begin{itemize}
    \item Dashboard
    \item Noticias
    \item Editais
    \item Defesas
    \item Estagios e Empregos
    \item Pessoal
    \item Menu Principal
    \item Pos-graduacao
    \item Acao de importacao do Processo Seletivo antigo (dentro de Pos-graduacao)
\end{itemize}

\section{Onde entrar para editar (secretaria)}
\subsection{Acesso admin}
\begin{itemize}
    \item URL: \texttt{/admin/login.php}
    \item Dashboard: \texttt{/admin/dashboard.php}
\end{itemize}

\subsection{Edicoes por modulo}
\begin{itemize}
    \item Noticias/Editais/Defesas/Estagios: \texttt{/admin/content.php?type=...}
    \item Pessoal (docentes/funcionarios): \texttt{/admin/pessoal.php}
    \item Menu principal (Graduacao/Pos-graduacao): \texttt{/admin/menu.php}
    \item Pos-graduacao (conteudo, editais da pos, egressos por ano): \texttt{/admin/pos-graduacao.php}
    \item Processo Seletivo antigo (importacao): botao \textit{Importar dados agora} em \texttt{/admin/pos-graduacao.php}
\end{itemize}

\section{Modulo de Pos-graduacao implementado}
Foi implementado um modulo interno completo no site do DECOM, sem dependencia de pagina externa, com:
\begin{itemize}
    \item pagina publica da pos em \texttt{/ensino/pos-graduacao.php};
    \item secoes editaveis no admin (ingresso, editais, grade, docencia, bolsas, facilidades para graduacao);
    \item listagem de formandos/egressos com paginacao por ano;
    \item editais/informes da pos com exibicao na propria pagina e na home.
    \item pagina dedicada de processo seletivo em \texttt{/ensino/pos-processo-seletivo.php}.
\end{itemize}

\section{Home page dinamica para a Pos}
A pagina inicial passou a exibir bloco da pos-graduacao com:
\begin{itemize}
    \item resumo institucional;
    \item ultimos editais/informes da selecao;
    \item link direto para a pagina completa da pos.
\end{itemize}
Todo esse conteudo e gerenciavel via painel admin.

\section{Pagina de Processo Seletivo (importacao da base antiga)}
Foi criada uma pagina especifica para replicar e consolidar o conteudo de:
\begin{center}
\texttt{https://www3.decom.ufop.br/pos/processoseletivo/}
\end{center}

\subsection{Implementacao}
\begin{itemize}
    \item pagina publica: \texttt{/ensino/pos-processo-seletivo.php};
    \item armazenamento estruturado em \texttt{ppgcc\_selection\_items};
    \item funcao de importacao: \texttt{ppgcc\_import\_selection\_page()} em \texttt{includes/config.php};
    \item execucao manual via admin para secretaria (botao de importacao).
\end{itemize}

\subsection{Conteudo importado}
Sao importados e agrupados por edital/selecao:
\begin{itemize}
    \item links de edital;
    \item formularios de inscricao;
    \item baremas e planilhas;
    \item comissoes;
    \item resultados preliminares e finais;
    \item anexos relacionados.
\end{itemize}

\section{Importacao historica de noticias do portal antigo}
Foi implementada a migracao automatica do acervo antigo:
\begin{center}
\texttt{https://www3.decom.ufop.br/decom/noticias/noticias/}
\end{center}

\subsection{Implementacao}
\begin{itemize}
    \item script: \texttt{app/scripts/import\_old\_news.php};
    \item varredura de paginas do acervo antigo com descoberta de links em \texttt{/acervo/...};
    \item parse de titulo, resumo, conteudo e data (quando identificavel);
    \item gravacao idempotente em \texttt{news\_items} (sem duplicacao por URL).
\end{itemize}

\subsection{Resultado consolidado da ultima importacao}
\begin{itemize}
    \item links candidatos identificados: 929;
    \item noticias inseridas/atualizadas na rodada: 874;
    \item itens ignorados/falhos: 55;
    \item total atual em \texttt{news\_items}: 1184 registros.
\end{itemize}

\subsection{Paginacao por ano (noticias)}
A pagina \texttt{/noticias/index.php} foi estruturada para:
\begin{itemize}
    \item listar anos disponiveis a partir de \texttt{YEAR(published\_at)};
    \item filtrar noticias por ano (\texttt{?ano=YYYY});
    \item paginar resultados por pagina (\texttt{?pagina=N}).
\end{itemize}

\section{Lista resumida de entregas realizadas}
\begin{itemize}
    \item correcoes de sessao/login admin e estabilidade de redirecionamento;
    \item padronizacao de cards para noticias, editais, defesas e estagios/empregos;
    \item modulo de pessoal com foto, perfil e campos editaveis;
    \item novos modulos de pesquisa (laboratorios e projetos);
    \item pagina institucional de Ciencia da Computacao e redirecionamentos solicitados;
    \item pagina de localizacao com mapa Google e informacoes de campus;
    \item pagina de mapa do campus com referencias visuais e links oficiais;
    \item menu principal editavel no admin (graduacao e pos-graduacao);
    \item modulo completo da pos-graduacao com egressos paginados por ano;
    \item pagina de processo seletivo da pos importada da base antiga;
    \item importacao massiva do acervo de noticias antigo para o novo portal.
\end{itemize}

\section{Seguranca: vulnerabilidades e mitigacoes}
\subsection{Vulnerabilidades identificadas (estado anterior)}
\begin{itemize}
    \item \textbf{Autenticacao administrativa centralizada em variavel de ambiente}: havia apenas uma credencial global, sem gestao de contas por usuario.
    \item \textbf{Ausencia de RBAC}: usuarios administrativos nao tinham segregacao de permissao por modulo.
    \item \textbf{Controle de tentativas de login fraco}: lockout baseado apenas em sessao, sem correlacao por IP/e-mail.
    \item \textbf{Sem trilha de auditoria persistente}: alteracoes administrativas nao eram registradas em tabela dedicada.
    \item \textbf{Headers HTTP de seguranca ausentes}: protecoes de browser (frame, MIME sniffing, CSP etc.) nao estavam padronizadas.
    \item \textbf{Politica de senha permissiva}: criacao e reset de senha com criterios minimos.
\end{itemize}

\subsection{Mitigacoes implementadas nesta etapa}
\begin{itemize}
    \item \textbf{RBAC completo no admin} com perfis:
    \begin{itemize}
        \item \texttt{superadmin}
        \item \texttt{editor}
        \item \texttt{secretaria}
    \end{itemize}
    aplicados por \texttt{require\_admin\_permission(...)} em cada modulo administrativo.
    \item \textbf{Gestao de contas administrativas} via \texttt{/admin/users.php}:
    \begin{itemize}
        \item criar, editar, ativar/desativar usuarios;
        \item redefinir senha;
        \item atribuir perfil.
    \end{itemize}
    \item \textbf{Novas tabelas de seguranca}:
    \begin{itemize}
        \item \texttt{admin\_users}
        \item \texttt{admin\_auth\_events}
        \item \texttt{admin\_audit\_logs}
    \end{itemize}
    \item \textbf{Rate limiting de login por IP/e-mail} com janela de 15 minutos (\texttt{admin\_auth\_events}).
    \item \textbf{Auditoria de acoes criticas}:
    \begin{itemize}
        \item login/logout;
        \item criacao/edicao/reset de usuarios;
        \item create/update/delete de conteudo;
        \item alteracao de menu e carrossel.
    \end{itemize}
    \item \textbf{Headers de seguranca HTTP} em todas as respostas:
    \begin{itemize}
        \item \texttt{X-Frame-Options}
        \item \texttt{X-Content-Type-Options}
        \item \texttt{Referrer-Policy}
        \item \texttt{Permissions-Policy}
        \item \texttt{Content-Security-Policy}
        \item \texttt{Strict-Transport-Security} (quando HTTPS)
    \end{itemize}
    \item \textbf{Politica de senha forte} para contas administrativas:
    minimo de 10 caracteres, com maiuscula, minuscula, numero e caractere especial.
    \item \textbf{Opcao ``lembrar senha'' no admin} implementada com cookie de sessao persistente (30 dias), com limpeza no logout.
\end{itemize}

\subsection{Risco residual e proximas acoes recomendadas}
\begin{itemize}
    \item substituir ``lembrar senha'' por \textbf{token proprio revogavel por dispositivo} (mais seguro que sessao persistente simples);
    \item adicionar \textbf{2FA} para perfil \texttt{superadmin};
    \item mover segredos para \textbf{gerenciador de secrets} (evitar credenciais em compose/ambiente sem cofre);
    \item implementar rotina de \textbf{backup com teste de restauracao} e trilha de conformidade;
    \item incluir varredura automatica de dependencias e teste de seguranca no pipeline de deploy.
\end{itemize}

\section{Racional de desempenho e baixo consumo}
As decisoes tecnicas priorizam ambiente institucional com recursos modestos:
\begin{itemize}
    \item consultas SQL diretas e simples;
    \item renderizacao server-side sem SPA pesada;
    \item baixa quantidade de dependencias de runtime;
    \item containerizacao simples (app + db + phpmyadmin);
    \item facilidade de backup e manutencao operacional.
\end{itemize}
Com isso, o portal permanece funcional, rapido e sustentavel em servidores com CPU e memoria limitados.

\section{Conclusao}
O portal DECOM foi evoluido para um modelo institucional dinamico, com administracao centralizada para secretaria, arquitetura limpa e baixa complexidade operacional. A solucao adota tecnologias livres ou com uso gratuito consolidado, mantendo foco em desempenho, simplicidade e autonomia de manutencao.

\section*{Como compilar este documento}
No diretorio do projeto:
\begin{verbatim}
pdflatex DOCUMENTACAO_ARQUITETURA_DECOM.tex
\end{verbatim}

\end{document}
